...
- **决策阈值自适应放宽**：C++ 可用时按实测提速校准起点（`part_solve` 组枚举上限
  +8（提速 ~424x），win_rate 阈值起点 6→8（提速 ~6x））；纯 Python 回退时自动
  恢复原值。设 `MSW_NATIVE_TUNE=0` 可强制保持原阈值（新旧实现决策对比时使用）。
- **总耗时预算控制（30s）与 win_rate 阈值实测反馈（无上限）**：`number5_1` 单次
  决策从入口计时；part_solve 按指数模型 `t(L) ≈ base·2^(L-33)` 预测各组耗时并
  逐次实测校准（状态跨决策记忆），预计超预算的组自动降级到未枚举区。win_rate
  阈值不设上限，由实测反馈在预算内自由升降：上次 win_rate 耗时 > 预算×0.75 →
  阈值降 1 档（回退 pbs）；连续 3 次 < 预算×0.25 → 阈值升 1 档（覆盖更大局面，
  结果更优）；控制器自然稳定在预算×0.25~0.75 区间。DEBUG 日志含各阶段耗时
  （枚举阶段 / 决策完成），可定位超时来源。
  win_rate 阈值反馈（用户规则）：步长 ±0.5、无上限、下限 0；升档仅在
  「真实 limitation 落在阈值 ±0.25 内（边界采样）且耗时 < 预算×25%」时发生；
  耗时 > 预算×75% 即降 0.5（安全阀，不要求边界，避免阈值升上去收不回）。
- **进度条双保险（决策级单调，绝不回退）**：① C++ 计算段释放 GIL（回调瞬间重取）
  ——长调用期间主线程可实时渲染真实进度（此前 GIL 被持有导致进度事件堆积、冻结
  至调用结束）；② 决策级单调坐标：整个 number5_1 的进度统一映射到一条单调刻度
  （预扫描 [0,10) → 枚举按各组预测耗时加权 [10,80) → 决策 [80,100]），低于已
  显示值的发射直接丢弃、收尾强制 100；速度估计心跳仅在真实进度静默时推进且同样
  受单调约束——进度条不会出现回退跳动。纯 Python 回退模式保持原始按操作 0-100
  语义。
