...
from PyQt5.QtWidgets import QMainWindow, QApplication, QMessageBox, QCheckBox, QPushButton

from edit_setting import Ui_Dialog
from mm0 import minesweeper_run, ClientToScreen, ScreenToClient, GetMousePosition, MouseWindowTread, set_top_window
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.plainTextEdit.verticalScrollBar().setStyleSheet("""QScrollBar:vertical
        {
        ...
        self.auto_play_thread.Visible_signal.connect(lambda v: self.end_auto_play_thread.setVisible(v))
        self.auto_play_thread.warning_signal.connect(lambda s: QMessageBox.warning(self, '错误', s))
        self.auto_play_thread.start_signal.connect(functools.partial(self.update_pgb, name='pgb'))
        ...
        self.help_thread.Visible_signal.connect(lambda v: self.end_help_thread.setVisible(v))
        self.help_thread.update_btn_list_signal.connect(self.update_btn_list)
        self.help_thread.warning_signal.connect(lambda s: QMessageBox.about(self, '提示', s))
        self.help_thread.warning_signal_2.connect(self.help_thread_warning)
        self.help_thread.start_signal.connect(functools.partial(self.update_pgb, name='pgb_p3'))
        ...
        for j in range(self.h):
            ...
            btn.setStyleSheet("QPushButton{border-radius:0px;border: 1px solid rgb(210, 210, 210);"
            ...
            for i in range(self.w):
                ...
                btn.setGeometry(i * self.btn_width + self.btn_width,
                                j * self.btn_width + self.btn_width,
                                self.btn_width,
                                self.btn_width)
                btn.setStyleSheet("QPushButton{border-radius:0px;border: none;"
                                  "background-color:rgba(255, 255, 255, 255);font: 9pt \"楷体\"}"
    ...
    def update_image(self):
        ...
        pil_img = ImageGrab.grab((self.bx + 0.5 * self.cell_width, self.by + 0.5 * self.cell_width,
                                  w + self.bx + 0.5 * self.cell_width, h + self.by + 0.5 * self.cell_width))
        pil_img.save('image/save.png')
        self.label_p3.setPixmap(QtGui.QPixmap('image/save.png').scaled(self.w * self.btn_width,
                                                                       self.h * self.btn_width))
    ...
    def update_btn_list(self, pos_dict_list):
        ...
        for pos_dict in pos_dict_list:
            ...
            if pos_dict['is_mine'] or confidence <= 0.01:
                ...
                self.btn_list[j][i].setStyleSheet("QPushButton{border-radius:0px; border: 2px solid rgb(0, 0, 255);"
                ...
                self.btn_list[j][i].setStyleSheet(f"QPushButton{{border-radius:0px; border: 3px solid rgb(255, 255,"
                ...
                self.btn_list[j][i].setStyleSheet(f"QPushButton{{border-radius:0px; border: none;"
    ...
    def info(self, s):
        ...
        if self.list_p:
            for (i, j) in self.list_p:
                ...
                j = int(j) - 1
                self.btn_list[j][i].setEnabled(True)
                self.btn_list[j][i].setStyleSheet("QPushButton{border-radius:0px; border: 3px solid rgb(211, 51, 255);"
                                                  "background-color:rgba(0, 0, 0, 0);font: 9pt \"楷体\"}"
                                                  "QPushButton:disabled{border: none;background-color:rgba(0, 0, 0, 0);"
                                                  "color:rgba(210, 210, 210, 0);}")
        self.info_window = MyMessageBox(self, message=s, x=self._x + self.x(), y=self._y + self.y())
    ...
    def help_thread_warning(self, s):
        if self.is_show:
            self.warning_window = MyMessageBox(self, title='警告', message=s, checked=True)
    ...
    def update_screen(self):
        ...
        elif self.auto_play_thread.isFinished() and self.stackedWidget.currentIndex() == 1:
            self.set_btns_Enabled(True)
        ...
        elif self.auto_play_thread.isFinished() and self.stackedWidget.currentIndex() == 2:
            self.set_btns_Enabled(True)
    ...
    def auto_play_func(self):
        # 启动自动程序
        self.reload()
        value, ok = QInputDialog.getInt(self, "总局数", "请输入要玩的总局数\n\n请输入整数:", 1, 1, 10000, 1)
        if ok:
            try:
...
class MyMessageBox(QMessageBox):
    ...
    def __init__(self, parent=None, title='消息', message='', x=None, y=None, checked=False):
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent) -> None:
        self._x = self.x()
        self._y = self.y()
        param = (self._x, self._y, self.cb.isChecked()) if self.cb else (self._x, self._y)
...
class EditWindow(QDialog, Ui_Dialog):
    def __init__(self):
        ...
        self.label_path.setText(str(self.path[-30:] if len(self.path) > 30 else self.path))
    ...
    def get_pos_2_func(self, pos):
        ...
        if abs(self.cell_width - cell_width) >= 0.11:
            ...
            self.mouse_window = None
            return
        self.bx, self.by = round(self.x0 - self.cell_width * 0.5), round(self.y0 - self.cell_width * 0.5)
    ...
    def get_int_limit(self):
        value, ok = QInputDialog.getInt(self, "limit",
                                        "limit\n注：但limit每增加1计算所需的时间增加1倍\n\n请输入整数:",
                                        self.limit, 0, 10000, 2)
        if ok:
...
class ScreenShot(QtWidgets.QWidget, Ui_form):
    def __init__(self, hwnd):
        ...
        self._name_list = ['0.bmp', '0_1.bmp', '0_2.bmp', '1.bmp', '1_1.bmp', '1_2.bmp', '2.bmp',
                           '2_1.bmp', '3.bmp', '3_1.bmp', '4.bmp', '4_1.bmp', '5.bmp', '5_1.bmp',
                           '6.bmp', '6_1.bmp', '7.bmp', '7_1.bmp', '8.bmp', '8_1.bmp', '9.bmp',
                           '9_1.bmp', '9_2.bmp', '10.bmp', '10_1.bmp']
